...
Order-summary update: staff/customer dashboards now receive allowlisted order data from /api/account-orders after Firebase token verification. Raw order documents are full-admin-only in the reviewed rules; Seller/customer direct Firestore reads are denied. This prevents the new dashboards returning legacy raw payment fields. Existing data still needs a deliberate migration/removal plan.
