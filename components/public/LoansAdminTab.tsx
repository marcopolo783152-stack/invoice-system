...
export default function LoansAdminTab(){
 ...
 const printReceipt=(p:Payment)=>{
  if(!selected)return;
  const html=`<!doctype html><html><head><title>Loan receipt</title><style>body{font-family:Arial;padding:40px;color:#111}h1{margin:0 0 6px}.box{border:1px solid #bbb;padding:22px;margin-top:24px}.row{display:flex;justify-content:space-between;padding:8px 0;border-bottom:1px solid #eee}.muted{color:#666}.sig{margin-top:70px;display:flex;gap:60px}.sig div{width:45%;border-top:1px solid #777;padding-top:8px}</style></head><body><h1>MARCO POLO ORIENTAL RUGS INC.</h1><div class="muted">Loan repayment receipt</div><div class="box"><div class="row"><b>Borrower</b><span>${selected.borrowerName}</span></div><div class="row"><b>Receipt</b><span>LN-${selected.id.slice(0,6).toUpperCase()}-${p.id.slice(-8).toUpperCase()}</span></div><div class="row"><b>Date</b><span>${p.date}</span></div><div class="row"><b>Amount paid</b><span>${money(p.amountCents)}</span></div><div class="row"><b>Method</b><span>${p.method||'-'}</span></div><div class="row"><b>Notes</b><span>${p.notes||'-'}</span></div></div><div class="sig"><div>Authorized signature</div><div>Borrower signature</div></div><script>window.onload=()=>window.print()</script></body></html>`;
