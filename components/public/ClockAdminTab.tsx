...
export function ClockAdminTab() {
    ...
    const handleClock = async (e: React.FormEvent, overrideIdentifier?: string, facePhotoUrl?: string) => {
        ...
        let finalPhoto = facePhotoUrl;
        
        // If they use manual PIN but webcam is on, snap a photo anyway
        if (!finalPhoto && streamRef.current !== null && videoRef.current) {
            try {
                const tmpCanvas = document.createElement('canvas');
                tmpCanvas.width = 160;
                tmpCanvas.height = 120;
                ...
                if (ctx) {
                    ...
                    ctx.scale(-1, 1);
                    ctx.drawImage(videoRef.current, 0, 0, 160, 120);
                    finalPhoto = tmpCanvas.toDataURL('image/jpeg', 0.5);
                }
            } catch (err) {
                console.error("Failed to capture manual photo", err);
            }
        }
