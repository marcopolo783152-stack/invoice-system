    ...
    <div className="bg-[#F9F7F5] min-h-screen py-12 font-sans text-xs">
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8 text-left">
        ...
        <div className="text-center space-y-2">
          <span className="text-[10px] uppercase tracking-[0.3em] text-editorial-accent font-bold block">Live Freight Logistics</span>
            ...
            {!activeOrder && !activeCleaning ? (
              <div className="bg-white p-10 rounded-none border border-editorial-border shadow-sm text-center space-y-3">
                ...
                <button
                  ...
                  className="px-4 py-2.5 bg-editorial-accent hover:bg-[#8E7453] text-white text-[10px] font-bold uppercase tracking-wider rounded-none transition"
                >
              ...
              <div className="space-y-6">
                ...
                <div className="bg-white p-6 rounded-none border border-editorial-border shadow-sm space-y-4">
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-editorial-border pb-4 gap-2">
                    <div>
                      <span className="text-[9px] uppercase tracking-wider text-gray-400 font-semibold block">Active Invoice Registry</span>
                      <h3 className="font-serif text-base font-light text-editorial-text">{activeOrder.id}</h3>
                    </div>
                    <div className="text-left sm:text-right">
                      <span className="text-[9px] uppercase tracking-wider text-gray-400 font-semibold block">Current Status</span>
                      <span className={`inline-block px-2.5 py-0.5 rounded-none text-[9px] font-bold uppercase tracking-wider border ${
                  ...
                  {activeOrder.status === "Cancelled" ? (
                    <div className="p-4 bg-red-50 border border-red-200 rounded-none text-red-800 space-y-1">
                      ...
                      <p className="text-[10px]">This transaction has been cancelled. For details or custom refund processing, contact our master advisors.</p>
                    </div>
                  ) : (
                    <div className="py-4 space-y-6">
                      <h4 className="text-[10px] uppercase tracking-widest text-editorial-accent font-bold">Curated Progress</h4>
                          ...
                          return (
                            <div key={idx} className="relative">
                              ...
                              <span className={`absolute -left-9 top-1.5 flex h-5 w-5 items-center justify-center rounded-none border transition ${
                                isCompleted ? "bg-editorial-accent border-editorial-accent text-white font-bold text-[9px]" :
                                isActive ? "bg-white border-editorial-accent text-editorial-accent ring-4 ring-[#C2B29F]/15 text-[9px] font-bold" :
                                "bg-white border-editorial-border text-gray-400 text-[9px]"
                              }`}>
                                {isCompleted ? "✓" : idx + 1}
                              </span>
                              ...
                              <div className="space-y-0.5">
                                ...
                                <p className={`text-[10px] ${isActive ? "text-gray-500 font-light" : "text-gray-400 font-light"}`}>
                                  {step.desc}
                                </p>
                              </div>
                            </div>
                  ...
                  {activeOrder.shippingDetails && (
                    <div className="p-5 bg-editorial-text rounded-none text-white border border-editorial-border space-y-3">
                      <div className="flex items-center gap-2 text-editorial-accent font-bold uppercase tracking-wider text-[10px]">
                        <Truck className="h-4.5 w-4.5" />
                        <span>Insured Carrier Dispatch Documents</span>
                      </div>
                      <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-[10px] border-t border-gray-700 pt-3">
                        <div>
                  ...
                  <div className="pt-4 border-t border-editorial-border flex flex-wrap gap-2 justify-between items-center">
                    <span className="text-[10px] text-gray-400 font-mono">Registered on: {new Date(activeOrder.createdAt).toLocaleString()}</span>
                    <button
                      onClick={handleContactSupport}
                      className="px-4 py-2 bg-editorial-aside border border-editorial-border hover:border-editorial-accent hover:text-editorial-accent rounded-none text-[10px] font-bold uppercase tracking-wider transition"
                    >
                ...
                {/* Invoice contents */}
                <div className="bg-white p-6 rounded-none border border-editorial-border shadow-sm space-y-4">
                  <h4 className="text-[10px] uppercase tracking-widest text-editorial-accent font-bold border-b border-editorial-border pb-2">Itemized Curation Invoice</h4>
                      ...
                      <div key={item.rug.id} className="flex gap-4 items-center justify-between py-2 border-b border-stone-50">
                        <div className="flex gap-3 items-center">
                          ...
                          <div className="text-left">
                            ...
                            <span className="text-[9px] text-gray-400">Dimensions: {item.rug.dimensions} | SKU: {item.rug.sku}</span>
                          </div>
                        </div>
                  ...
                  <div className="text-[10px] text-gray-500 space-y-1 bg-editorial-aside p-4 rounded-none border border-editorial-border text-left">
                  ...
                  <div className="text-[9px] text-gray-400 space-y-1 border-t border-editorial-border pt-3 text-left font-light">
              ...
              <div className="space-y-6">
                ...
                <div className="bg-white p-6 rounded-none border border-editorial-border shadow-sm space-y-4">
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-editorial-border pb-4 gap-2">
                    <div>
                      <span className="text-[9px] uppercase tracking-wider text-gray-400 font-semibold block">Specialty Care Registry</span>
                  ...
                  {activeCleaning.status === "Cancelled" ? (
                    <div className="p-4 bg-red-50 border border-red-200 rounded-none text-red-800 space-y-1">
                      ...
                      <p className="text-[10px]">This specialty service booking has been cancelled. Contact our master advisors for more information.</p>
                    </div>
                  ) : (
                    <div className="py-4 space-y-6">
                      <h4 className="text-[10px] uppercase tracking-widest text-editorial-accent font-bold">Service Progress</h4>
                  ...
                  {/* Actions row */}
                  <div className="pt-4 border-t border-editorial-border flex flex-wrap gap-2 justify-between items-center">
                    <span className="text-[10px] text-gray-400 font-mono">Booked on: {new Date(activeCleaning.createdAt).toLocaleString()}</span>
                    <button
                      onClick={() => {
                ...
                {/* Booking details */}
                <div className="bg-white p-6 rounded-none border border-editorial-border shadow-sm space-y-4">
                  <h4 className="text-[10px] uppercase tracking-widest text-editorial-accent font-bold border-b border-editorial-border pb-2">Service Details</h4>
