...
const sendGridBlockRegex = /\/\/ 1\. SendGrid Email Notification[\s\S]*?(?=\/\/ 2\. Twilio SMS Notification)/;

const newEmailJSBlock = `// 1. EmailJS Notification (Replaced SendGrid to bypass DMARC issues)
    const EMAILJS_SERVICE_ID = 'marcopolo2'; // From your default config
    const EMAILJS_TEMPLATE_ID = 'rm8govh'; // Your existing signature template
      ...
      try {
        ...
        const emailJsPayload = {
          ...
          template_params: {
            ...
            subject: subjectLine,
            message: emailHtml,
            // Fallbacks in case the signature template requires these specific variables
            email_title: subjectLine,
        ...
        results.email = ejsResponse.ok ? 'Sent' : \`Failed: \${ejsResponse.status}\`;
      } catch (err) {
        results.email = 'Error';
      }
    } else {
      results.email = 'No Email provided';
    }

    `;

if (sendGridBlockRegex.test(content)) {
    content = content.replace(sendGridBlockRegex, newEmailJSBlock);
    fs.writeFileSync(routeFile, content);
    console.log('Successfully replaced SendGrid with EmailJS REST API in notify-order route.');
} else {
