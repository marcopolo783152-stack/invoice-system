...
export async function getEmployees(): Promise<Employee[]> {
    ...
    if (isFirebaseConfigured() && db) {
        try {
            ...
            // LocalStorage caching removed to enforce direct Firebase usage
            return employees;
        } catch (e: any) {
...
export async function saveEmployee(employee: Partial<Employee>): Promise<Employee> {
    ...
    // Local cache updates removed. We rely strictly on Firebase data streams.
...
export async function checkAutoClockOut(): Promise<number> {
    ...
    if (count > 0) {
        ...
        const localEmpKey = getKey(BASE_LOCAL_EMP_KEY);
        // localStorage.setItem(, JSON.stringify(employees));
    }
...
export async function addManualTimeLog(log: Omit<TimeLog, 'id'>): Promise<TimeLog> {
    ...
    try {
        ...
        if (idx >= 0) {
            ...
            // localStorage.setItem(, JSON.stringify(localEmployees));
        }
    } catch (e) {
...
export async function updateTimeLog(logId: string, updates: Partial<TimeLog>): Promise<void> {
    ...
    if (idx >= 0) {
        ...
        // localStorage.setItem(, JSON.stringify(localLogs));
    }
}
