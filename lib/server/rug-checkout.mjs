import {checkoutRegionAllowed, checkoutOriginKnown} from './checkout-region.mjs';
...
export function checkoutInput(body) {
  ...
  Object.assign(customerInfo, {billingCountry: c.billingCountry, shippingCountry: c.shippingCountry});
  if (!checkoutRegionAllowed(customerInfo)) throw new CheckoutError('Sandbox checkout requires U.S. billing and delivery countries and no restricted-region addresses.', 403);
...
export function priceCart(input, rugs, promo, now = Date.now()) {
  const items = input.items.map(({id}) => {
    ...
    if (isRugOnReviewHold(rug) || !checkoutOriginKnown(rug) || !rug || rug.availability !== 'In Stock') throw new CheckoutError('A rug is no longer available. Please refresh your cart.', 409);
...
export function rugSessionParams(order, origin) {
  if (!checkoutRegionAllowed(order.customerInfo)) throw new CheckoutError('Order locations require review.', 403);
