...
export function checkoutParams(uid, origin) {
  return new URLSearchParams({
    ...
    'line_items[0][price_data][product_data][name]': 'Marco Polo — TEST ONLY — no purchase',
...
export function sessionResult(session, uid) {
  if (session.livemode !== false || !session.id?.startsWith('cs_test_') ||
    ...
    throw new StripeTestError('This is not a matching Marco Polo test payment.', 403);
  }
